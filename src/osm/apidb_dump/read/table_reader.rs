    ...
    fn create_node(line: &str, table_def: &TableDef, line_number: usize) -> Option<TableRecord> {
        ...
        match table_def.fields_ref() {
            TableFields::Nodes { node_id, latitude, longitude, changeset_id, visible, timestamp, tile, version, redaction_id } => {
                ...
                assert!(*latitude < columns.len(), "column {} for field (latitude) is missing in {}:{}", *latitude + 1, table_def.path().display(), line_number);
                assert!(*longitude < columns.len(), "column {} for field (longitude) is missing in {}:{}", *longitude + 1, table_def.path().display(), line_number);
                assert!(*changeset_id < columns.len(), "column {} for field (changeset_id) is missing in {}:{}", *changeset_id + 1, table_def.path().display(), line_number);
                assert!(*visible < columns.len(), "column {} for field (visible) is missing in {}:{}", *visible + 1, table_def.path().display(), line_number);
                assert!(*timestamp < columns.len(), "column {} for field (timestamp) is missing in {}:{}", *timestamp + 1, table_def.path().display(), line_number);
                assert!(*tile < columns.len(), "column {} for field (tile) is missing in {}:{}", *tile + 1, table_def.path().display(), line_number);
                assert!(*version < columns.len(), "column {} for field (version) is missing in {}:{}", *version + 1, table_def.path().display(), line_number);
                assert!(*redaction_id < columns.len(), "column {} for field (redaction_id) is missing in {}:{}", *redaction_id + 1, table_def.path().display(), line_number);
                Some(
                    TableRecord::Node {
    ...
    fn create_node_tag(line: &str, table_def: &TableDef, line_number: usize) -> Option<TableRecord> {
        ...
        match table_def.fields_ref() {
            TableFields::NodeTags { node_id, version, k, v } => {
                assert!(*node_id < columns.len(), "column {} for field (node_id) is missing in {}:{}", *node_id + 1, table_def.path().display(), line_number);
                assert!(*version < columns.len(), "column {} for field (version) is missing in {}:{}", *version + 1, table_def.path().display(), line_number);
                assert!(*k < columns.len(), "column {} for field (k) is missing in {}:{}", *k + 1, table_def.path().display(), line_number);
                assert!(*v < columns.len(), "column {} for field (v) is missing in {}:{}", *v + 1, table_def.path().display(), line_number);
                Some(
                    TableRecord::NodeTag {
    ...
    fn create_way_node(line: &str, table_def: &TableDef, line_number: usize) -> Option<TableRecord> {
        ...
        match table_def.fields_ref() {
            TableFields::WayNodes { way_id, node_id, version, sequence_id } => {
                ...
                assert!(*node_id < columns.len(), "column {} for field (node_id) is missing in {}:{}", *node_id + 1, table_def.path().display(), line_number);
                assert!(*version < columns.len(), "column {} for field (version) is missing in {}:{}", *version + 1, table_def.path().display(), line_number);
                assert!(*sequence_id < columns.len(), "column {} for field (sequence_id) is missing in {}:{}", *sequence_id + 1, table_def.path().display(), line_number);
                Some(
                    TableRecord::WayNode {
    ...
    fn create_relation_member(line: &str, table_def: &TableDef, line_number: usize) -> Option<TableRecord> {
        ...
        match table_def.fields_ref() {
            TableFields::RelationMembers { relation_id, member_type, member_id, member_role, version, sequence_id } => {
                assert!(*relation_id < columns.len(), "column {} for field (relation_id) is missing in {}:{}", *relation_id + 1, table_def.path().display(), line_number);
                assert!(*member_type < columns.len(), "column {} for field (member_type) is missing in {}:{}", *member_type + 1, table_def.path().display(), line_number);
                assert!(*member_id < columns.len(), "column {} for field (member_id) is missing in {}:{}", *member_id + 1, table_def.path().display(), line_number);
                assert!(*member_role < columns.len(), "column {} for field (member_role) is missing in {}:{}", *member_role + 1, table_def.path().display(), line_number);
                assert!(*version < columns.len(), "column {} for field (version) is missing in {}:{}", *version + 1, table_def.path().display(), line_number);
                assert!(*sequence_id < columns.len(), "column {} for field (sequence_id) is missing in {}:{}", *sequence_id + 1, table_def.path().display(), line_number);
                Some(
    ...
    fn create_changeset(line: &str, table_def: &TableDef, line_number: usize) -> Option<TableRecord> {
        ...
        match table_def.fields_ref() {
            TableFields::Changesets { id, user_id, created_at, min_lat, max_lat, min_lon, max_lon, closed_at, num_changes } => {
                ...
                assert!(*user_id < columns.len(), "column {} for field (user_id) is missing in {}:{}", *user_id + 1, table_def.path().display(), line_number);
                assert!(*created_at < columns.len(), "column {} for field (created_at) is missing in {}:{}", *created_at + 1, table_def.path().display(), line_number);
                assert!(*min_lat < columns.len(), "column {} for field (min_lat) is missing in {}:{}", *min_lat + 1, table_def.path().display(), line_number);
                assert!(*max_lat < columns.len(), "column {} for field (max_lat) is missing in {}:{}", *max_lat + 1, table_def.path().display(), line_number);
                assert!(*min_lon < columns.len(), "column {} for field (min_lon) is missing in {}:{}", *min_lon + 1, table_def.path().display(), line_number);
                assert!(*max_lon < columns.len(), "column {} for field (max_lon) is missing in {}:{}", *max_lon + 1, table_def.path().display(), line_number);
                assert!(*closed_at < columns.len(), "column {} for field (closed_at) is missing in {}:{}", *closed_at + 1, table_def.path().display(), line_number);
                assert!(*num_changes < columns.len(), "column {} for field (num_changes) is missing in {}:{}", *num_changes + 1, table_def.path().display(), line_number);
                Some(
    ...
    fn create_user(line: &str, table_def: &TableDef, line_number: usize) -> Option<TableRecord> {
        ...
        match table_def.fields_ref() {
            TableFields::Users { email, id, pass_crypt, creation_time, display_name, data_public, description, home_lat, home_lon, home_zoom, pass_salt, email_valid, new_email, creation_ip, languages, status, terms_agreed, consider_pd, auth_uid, preferred_editor, terms_seen, description_format, changesets_count, traces_count, diary_entries_count, image_use_gravatar, auth_provider, home_tile, tou_agreed, } => {
                ...
                assert!(*id < columns.len(), "column {} for field (id) is missing in {}:{}", *id + 1, table_def.path().display(), line_number);
                assert!(*pass_crypt < columns.len(), "column {} for field (pass_crypt) is missing in {}:{}", *pass_crypt + 1, table_def.path().display(), line_number);
                assert!(*creation_time < columns.len(), "column {} for field (creation_time) is missing in {}:{}", *creation_time + 1, table_def.path().display(), line_number);
                assert!(*display_name < columns.len(), "column {} for field (display_name) is missing in {}:{}", *display_name + 1, table_def.path().display(), line_number);
                assert!(*data_public < columns.len(), "column {} for field (data_public) is missing in {}:{}", *data_public + 1, table_def.path().display(), line_number);
                assert!(*description < columns.len(), "column {} for field (description) is missing in {}:{}", *description + 1, table_def.path().display(), line_number);
                assert!(*home_lat < columns.len(), "column {} for field (home_lat) is missing in {}:{}", *home_lat + 1, table_def.path().display(), line_number);
                assert!(*home_lon < columns.len(), "column {} for field (home_lon) is missing in {}:{}", *home_lon + 1, table_def.path().display(), line_number);
                assert!(*home_zoom < columns.len(), "column {} for field (home_zoom) is missing in {}:{}", *home_zoom + 1, table_def.path().display(), line_number);
                assert!(*pass_salt < columns.len(), "column {} for field (pass_salt) is missing in {}:{}", *pass_salt + 1, table_def.path().display(), line_number);
                assert!(*email_valid < columns.len(), "column {} for field (email_valid) is missing in {}:{}", *email_valid + 1, table_def.path().display(), line_number);
                assert!(*new_email < columns.len(), "column {} for field (new_email) is missing in {}:{}", *new_email + 1, table_def.path().display(), line_number);
                assert!(*creation_ip < columns.len(), "column {} for field (creation_ip) is missing in {}:{}", *creation_ip + 1, table_def.path().display(), line_number);
                assert!(*languages < columns.len(), "column {} for field (languages) is missing in {}:{}", *languages + 1, table_def.path().display(), line_number);
                assert!(*status < columns.len(), "column {} for field (status) is missing in {}:{}", *status + 1, table_def.path().display(), line_number);
                assert!(*terms_agreed < columns.len(), "column {} for field (terms_agreed) is missing in {}:{}", *terms_agreed + 1, table_def.path().display(), line_number);
                assert!(*consider_pd < columns.len(), "column {} for field (consider_pd) is missing in {}:{}", *consider_pd + 1, table_def.path().display(), line_number);
                assert!(*auth_uid < columns.len(), "column {} for field (auth_uid) is missing in {}:{}", *auth_uid + 1, table_def.path().display(), line_number);
                assert!(*preferred_editor < columns.len(), "column {} for field (preferred_editor) is missing in {}:{}", *preferred_editor + 1, table_def.path().display(), line_number);
                assert!(*terms_seen < columns.len(), "column {} for field (terms_seen) is missing in {}:{}", *terms_seen + 1, table_def.path().display(), line_number);
                assert!(*description_format < columns.len(), "column {} for field (description_format) is missing in {}:{}", *description_format + 1, table_def.path().display(), line_number);
                assert!(*changesets_count < columns.len(), "column {} for field (changesets_count) is missing in {}:{}", *changesets_count + 1, table_def.path().display(), line_number);
                assert!(*traces_count < columns.len(), "column {} for field (traces_count) is missing in {}:{}", *traces_count + 1, table_def.path().display(), line_number);
                assert!(*diary_entries_count < columns.len(), "column {} for field (diary_entries_count) is missing in {}:{}", *diary_entries_count + 1, table_def.path().display(), line_number);
                assert!(*image_use_gravatar < columns.len(), "column {} for field (image_use_gravatar) is missing in {}:{}", *image_use_gravatar + 1, table_def.path().display(), line_number);
                assert!(*auth_provider < columns.len(), "column {} for field (auth_provider) is missing in {}:{}", *auth_provider + 1, table_def.path().display(), line_number);
                assert!(*home_tile < columns.len(), "column {} for field (home_tile) is missing in {}:{}", *home_tile + 1, table_def.path().display(), line_number);
                assert!(*tou_agreed < columns.len(), "column {} for field (tou_agreed) is missing in {}:{}", *tou_agreed + 1, table_def.path().display(), line_number);
                Some(
                    TableRecord::User {
